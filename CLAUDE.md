...
- 命令が実行可能かどうかの判定は，`alu.svh`の`is_instruction_executable()`(先読みデコードに対する判定)と`alu_sv.sv`のCPU_CHECKフェーズ(実行直前の判定)の2箇所にある．命令タイプ・命令コード・レジスタ番地の条件を変える場合は必ず両方を同時に直すこと(前者だけが緩くなると，不正な命令が強制リセットされずそのまま実行される)．
