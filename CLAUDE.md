...
CPUの命令の動作は，合成せずVivado付属のシミュレータで検証できる．Vivadoのbinディレクトリ(`xvlog`・`xelab`・`xsim`)にPATHを通した上で，Git Bashから`bash mypc.srcs/sim_1/run_cpu_tb.sh`を実行する．全テストケースが合格すれば終了コード0で終わり，不合格があればその内容を表示して0以外で終わる．引数に`gui`を付けると，実行せずに波形ビューア付きのシミュレータを開く．コンパイル結果とログは`mypc.sim/cpu_tb/`(Git管理外)に出力される．
