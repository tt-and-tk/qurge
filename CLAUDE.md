...
| `mypc.srcs/pn/` | ROM上で動くPynesisソース(`.pn`)．`compiler`の`c2asm.exe`でアセンブリへ，`assembler`の`asm2sv.exe`で`mypc.srcs/sources_1/new/rom_sv.sv`へ変換する(中間生成物の`.pt`は`.gitignore`対象) |
