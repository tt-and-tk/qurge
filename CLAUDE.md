...
- 命令が実行可能かどうかの判定は`alu.svh`の`is_instruction_executable()`が唯一の実装であり，先読みデコードに対する判定(`advance_to_next_instruction()`)とCPU_CHECKフェーズの判定の両方がこれを呼ぶ．命令タイプ・命令コード・レジスタ番地の条件を変える場合はこの関数だけを直すこと(CPU_CHECK側に条件を書き足すと判定が二重管理になり，先読み側だけが緩いまま残ると不正な命令が停止されずに実行される)．CPU_CHECKフェーズは「実行可能なら値を取り込んでEXECUTEへ，そうでなければ停止」だけを行う．ただしこの関数が命令コードの妥当性まで見ているのはJ系・M系・IO系だけで，P系・S系・A系・F系の未定義の命令コードはCPU_EXECUTEの`default`で停止する(2段構えになっている)．
- `alu_sv.sv`の`is_halted`は，実行できない命令を検出して停止していることを表すフラグ．立っている間はリセット処理の分岐が毎サイクル働き続けるため，このフラグを下ろす代入をリセット処理の中以外に置かないこと(外に置くと，停止した状態から自力で抜けてプログラムの先頭から同じ命令を踏み直す動作になる)．下ろすのは外部からのリセットが入っているときだけ．そのリセット信号はPSが出すもの(`FCLK_RESET0_N`)を`proc_sys_reset`で整形したものであり，PL側に配線されたボタンからは入らないため，実機での復帰手段はPS側からのオーバーレイ再ロードになる．
- `alu_sv.sv`のリセット処理は，順序回路が持つ状態を漏れなく初期状態へ戻す唯一の箇所．新しくフリップフロップを追加する場合は，ここでの初期化も必ず追加すること(特にAXI-Streamのハンドシェイク出力のような外部へ出る信号は，初期化を忘れると復帰後に転送の途中から再開したように振る舞う)．
- `is_halted`によるリセットが効くのは`alu_sv.sv`の中だけで，RAM・割り算IP・DMAは外部からのリセット信号でしか初期化されない．そのため，これらとのやり取りが完了していない状態で`is_halted`を立ててはならない(CPU側だけが待機状態へ戻り，相手は応答を待ったまま取り残される)．現状はメモリ・標準入出力・割り算の応答待ちの最中に`force_reset`へ落ちる経路がなく成立しているが，命令や検査を追加する際はこの前提を崩さないこと．
