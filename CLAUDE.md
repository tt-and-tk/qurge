...
- 命令が実行可能かどうかの判定は`alu.svh`の`is_instruction_executable()`が唯一の実装であり，先読みデコードに対する判定(`advance_to_next_instruction()`)とCPU_CHECKフェーズの判定の両方がこれを呼ぶ．命令タイプ・命令コード・レジスタ番地の条件を変える場合はこの関数だけを直すこと(CPU_CHECK側に条件を書き足すと判定が二重管理になり，先読み側だけが緩いまま残ると不正な命令が強制リセットされずに実行される)．CPU_CHECKフェーズは「実行可能なら値を取り込んでEXECUTEへ，そうでなければ強制リセット」だけを行う．
