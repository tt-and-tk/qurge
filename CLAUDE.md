...
| `qurge`(本リポジトリ) | `mypc/` | CPU・メモリ・ROM等のハードウェア全体のVivadoプロジェクト(SystemVerilog + PS側C++)と，ROM上で動く自作OS Qosmos(シェル・ファイルシステム・SDカードドライバ．Pynesisで記述) |
...
| `mypc.srcs/pn/` | ROM上で動くQosmosのPynesisソース(`.pn`)．`compiler`の`pn2asm.exe`でアセンブリへ，`assembler`の`asm2sv.exe`で`mypc.srcs/sources_1/new/rom_sv.sv`へ変換する(中間生成物の`.pt`は`.gitignore`対象) |
