...
- `alu_sv.sv`の`force_reset`は，実行できない命令を検出して停止していることを表すフラグ．立っている間はリセット処理の分岐が毎サイクル働き続けるため，このフラグを下ろす代入をリセット処理の中以外に置かないこと(外に置くと，停止した状態から自力で抜けてプログラムの先頭から同じ命令を踏み直す動作になる)．下ろすのは外部からのリセットが入っているときだけで，実機での復帰手段はボードのリセット，またはPS側からのオーバーレイ再ロードになる．
...
- `force_reset`によるリセットが効くのは`alu_sv.sv`の中だけで，RAM・割り算IP・DMAは外部からのリセット信号でしか初期化されない．そのため，これらとのやり取りが完了していない状態で`force_reset`を立ててはならない(CPU側だけが待機状態へ戻り，相手は応答を待ったまま取り残される)．現状はメモリ・標準入出力・割り算の応答待ちの最中に`force_reset`へ落ちる経路がなく成立しているが，命令や検査を追加する際はこの前提を崩さないこと．
