...
チェックポイントのパスは，utils_1ファイルセットの登録とsynth_1の`IncrementalCheckpoint`属性の2箇所とも`$PSRCDIR/utils_1/imports/synth_1/top_wrapper.dcp`が正しい値である．\
記録された場所(`Path`属性)と異なるディレクトリで開くと，Vivadoは手元に無いチェックポイントを記録された場所のものに置き換え，`$PPRDIR/..`でプロジェクトの外をたどるパスに書き換える．このパスは開く場所の深さで指す先が変わり，他の場所では合成が止まるため，コミット前に`$PPRDIR/..`を含む行が無いことを確かめる．
