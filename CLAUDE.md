...
デザインチェックポイントがGit管理外であることにより，プロジェクトを開いた直後は増分合成用のチェックポイントが見つからない旨の警告が出るが，最初の合成で生成されるため対処は不要。

`mypc.xpr`はプロジェクトの絶対パスと各ソースの取り込み時刻を保持しており，開くだけで内容が書き換わる。ソースを変更していないのに生じたこの差分はコミットしない。
...
ブロックダイアグラムとトップラッパーは，他のソースと同様にGit管理下に置く。これらが無いとチェックアウトしたディレクトリでプロジェクトを開けず、合成もできないためである。編集はGUI操作で行い、ファイルを直接書き換えないという扱いは変わらない。デザインチェックポイントは合成・実装をやり直せば再生成されるためGit管理外とする。

`mypc.srcs/sim_1/`はテストベンチを置く場所で，Vivadoが自動生成するものではないためGit管理の対象とする。現時点ではテストベンチが存在せず空である。
