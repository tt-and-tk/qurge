...
- CPU命令フォーマット・レジスタ番地などのISA仕様 → `../specification/isa.md`・`../specification/register.md`(このリポジトリ内には転記しない．唯一の一次情報源)
- Vivadoプロジェクトのモジュール階層・I/O配線・ファイル一覧 → `docs/architecture.md`
- 残件・既知課題(ハードウェア) → `docs/known-issues.md`
- `alu_sv.sv`等，個別のハードウェア実装上の注意点は該当ソースファイルのコメントを参照(CLAUDE.mdには集約しない方針．詳細はコミット`8b0e395`を参照)
