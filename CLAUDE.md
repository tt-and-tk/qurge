...
CPU命令フォーマット・レジスタ番地などのISA仕様は`../specification/index.md`を参照(唯一の一次情報源．このリポジトリ内には転記しない)。Vivadoプロジェクトのモジュール階層・I/O配線はソースファイル・ブロックダイアグラム自体を参照。`alu_sv.sv`等，個別のハードウェア実装上の注意点は該当ソースファイルのコメントを参照(CLAUDE.mdには集約しない方針．詳細はコミット`8b0e395`を参照)。残件・既知課題はGitHub issueで管理する(ドキュメントには集約しない)。
